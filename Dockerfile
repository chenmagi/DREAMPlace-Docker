...
COPY busydocker/post_build.sh /post_build.sh
COPY busydocker/include.pkg /include.pkg
COPY busydocker/user.dat /user.dat
COPY busydocker/ssh_key_busydocker.pu[b] /
RUN chmod +x /post_build.sh

RUN /post_build.sh -v 
# RUN rm /post_build.sh /user.dat  /ssh_key_busydocker.*
